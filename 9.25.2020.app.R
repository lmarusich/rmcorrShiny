...
server <- function(input, output, session) {
  ...
  plotdata <- eventReactive(input$plot, {
    ggplot2::ggplot(df(), ggplot2::aes(x = !!(sym(input$xvar)), y = !!(sym(input$yvar)), group = (!!(sym(input$subvar))),
      ...
      ggplot2::scale_colour_gradientn(colours=(eval(as.name(input$palette)))(length(unique(data()$model$model$Participant)))) #Add button to select palette next? 
      #+ (legend.position = "none")
    })
  
  
  output$rmcplot = renderPlot({plotdata()  + ggplot2::theme(legend.position = "none")})                         
  
  # data2 <- eventReactive(input$legend, {
  #     if(input$legend == F) {renderPlot({plotdata()  + ggplot2::theme(legend.position = "none")})     
  #     }
  #   else {renderPlot({plotdata()})     }
  #   })
